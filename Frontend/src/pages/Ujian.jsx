...
const Ujian = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await fetch(`${import.meta.env.VITE_API_SERVER}/api/questions`, {
        headers: {
  ...
  const handleStart = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_SERVER}/api/validate-token`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "ngrok-skip-browser-warning": "true",
  ...
  return (
    ...
    <div className="p-6 bg-white rounded-md shadow-md max-w-4xl mx-auto mt-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Pilih Ujian</h2>

      <div className="mb-4">
        <select value={kelas} onChange={(e) => setKelas(e.target.value)} className="border px-4 py-2 mr-4">
        ...
        <select value={jenisUjian} onChange={(e) => setJenisUjian(e.target.value)} className="border px-4 py-2 mr-4">
          <option value="">Pilih Jenis Ujian</option>
          <option value="UTS">UTS</option>
          <option value="UAS">UAS</option>
        </select>

        <select value={mapel} onChange={(e) => setMapel(e.target.value)} className="border px-4 py-2">
      ...
      {filteredUjians.length === 0 ? (
        <p className="text-gray-500 text-center">Tidak ada ujian yang tersedia dengan kriteria ini.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredUjians.map((ujian) => (
            <div key={ujian.id} className="bg-white p-4 border rounded-md shadow-md">
              <h3 className="font-bold">{ujian.nama}</h3>
              <p>Kelas: {ujian.kelas}</p>
              <p>Jenis Ujian: {capitalize(ujian.jenis)}</p>
              <p>Mapel: {capitalize(ujian.mapel)}</p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg w-full max-w-md">
            <h2 className="text-lg font-semibold mb-4">Konfirmasi Ujian</h2>
            <input
              type="text"
              placeholder="Nama"
              value={nama}
